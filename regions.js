$(document).ready(function () {
   ...
   const regions = [
         ...
         Centres:  [
            {
               ...
               year: '2022',
               name: 'Муниципальное автономное общеобразовательное учреждение «Средняя общеобразовательная школа No 6» муниципального района «Город Краснокаменск и Краснокаменский район',
               address: 'Забайкальский край, Краснокаменский район,  г. Краснокаменск, 4-й микрорайон, дом 453',
               site: 'https://sh6-kr.edusite.ru/p110aa1.html',
               social: 'https://sh6-kr.edusite.ru/p110aa1.html'
            },
            {
               id: 2,
               federalDistrict: 'YUFO',
      ...
      },
      {
         ...
         Centres: [
            ...
            {
               id: 32,
               region: 'Республика Башкортостан - последнее!',
               year: '2022',
               name: 'Муниципальное автономное общеобразовательное учреждение «Средняя общеобразовательная школа No 6» муниципального района «Город Краснокаменск и Краснокаменский район',
               address: 'Забайкальский край, Краснокаменский район,  г. Краснокаменск, 4-й микрорайон, дом 453',
               site: 'https://sh6-kr.edusite.ru/p110aa1.html',
   ...
   const districts = [
      ...
      {
         text: 'Дальневосточный федеральный округ',
         id: 2,
         federalDistrict: 'DFO'
      },
      {
      ...
      {
         text: 'Сибирский федеральный округ',
         id: 4,
         federalDistrict: 'SFO'
      },
      {
      ...
      {
         text: 'Южный федеральный округ',
         id: 6,
         federalDistrict: 'YUFO'
      },
      {
         text: 'Северо-кавказский федеральный округ',
         id: 7,
         federalDistrict: 'SKFO'
      },
      {
   ...
   function initMap(regions) {
      ...
      $.each(regions, function (index, data) {
         ...
         newpath.setAttribute("data-total", data.total);
         newpath.setAttribute("stroke", '#ebf1f6');
         newpath.setAttribute("stroke-width", '.4');
         // newpath.setAttribute("fill", "#0272B0");
         switch (data.federalDistrict) {
            case 'CFO':
               newpath.setAttribute("fill", "#4F54CE");
               break;
            case 'DFO':
               newpath.setAttribute("fill", "#658CDA");
               break;
            case 'SFO':
               newpath.setAttribute("fill", "#67DC63");
               break;
            case 'YFO':
               newpath.setAttribute("fill", "#F5C242");
               break;
            case 'SZFO':
               newpath.setAttribute("fill", "#4FAD5B");
               break;
            case 'PFO':
               newpath.setAttribute("fill", "#5B20A6");
               break;
            case 'YUFO':
               newpath.setAttribute("fill", "#F1A13C");
               break;
            case 'SKFO':
               newpath.setAttribute("fill", "#BE5A27");
               break;
            default:
               newpath.setAttribute("fill", "#80A5C8");
               break;
         }
...
//   ___ ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ ___
   
   let $select_clear = $('.select__clear'),
       $back__holder = $('.back__holder'),
       $main_list_holder = $('#listed__holder');
   let $h1 = $('.listed__caption h1'),
       $h2 = $('.listed__caption h2 strong'),
       $word = $('.listed__caption span');
      ...
      let center = []; // Очищаем переменные
      let article = [],
          total = [],
          total_amount = 0;
      
      let all_regions = 'Все центры';
   ...
   function clear() {
      ...
      showCenters( regions, all_regions ); // ОТРИСОВКА ВСЕХ ЦЕНТРОВ ВО ВСЕХ РЕГИОНАХ
      
      $districts.prop("disabled", false);
      $regions.prop("disabled", false);
      
      $select_clear.addClass('disable');
      $back__holder.hide();
      
      $('.chosen_region').removeClass('chosen_region');
   }
   $select_clear.on( 'click', clear);
   $back__holder.on( 'click', clear);
   
   function clearListed() {
      $('#listed__holder article').remove('article');
      $h1.text('');
      $h2.text('');
      $word.text('');
   }
   
   function colorDistrict( district ) { // При наведении на район, подсветим его ФО на круговой диаграмме сверху
      
      $('path[data-district^=' + district + ']').on( 'mouseenter', function () {
         $('#main').css( 'opacity', '0');
         $('#' + district).addClass('show-info');
      }).on( 'mouseleave', function () {
         $('#main').css( 'opacity', '1');
         $('#' + district).removeClass('show-info');
      })
   }
   colorDistrict( 'CFO' );
   colorDistrict( 'DFO' );
   ...
   // При наведении на район, должна появляться карточка с названием регионов и числом центров там
   $('#map_index__svg path').on( 'mouseenter', function( event ) {
      let region = $(event.target);
      let region_title = $(event.target).data( 'title' ),
   ...
   function showCardWithTotal( region, region_title, total ) {
      ...
      let $card = $('.card_total__container'),
          $name = $('.card_total__container h1'),
          $h2 = $('.card_total__container h2'),
          $total = $('.card_total__container h2 strong'),
          $word = $('.card_total__container h2 span'),
          $nocenter = $('.card_total__container .nocenter'),
          $icon = $('.icon-more');
      
      if ( region_total <= 0 ) {
         showCard( region, region_title, $name, $card );
         $icon.hide();
         $h2.hide();
         $nocenter.show();
         $nocenter.text('В данном регионе нет центров «IT-куб»');
   ...
   function getNoun(number, one, two, five) { // Склонение слов
  ...
  /*function list( data_region, data_year, data_name, data_address, data_site, data_social, fd_name, fd_ru) {
      hidePagination();
      
      region = $('<div><strong>Регион: </strong>' + data_region + '</div>');
      year = $('<div><strong>Год создания: </strong>' + data_year + '</div>');
      name = $('<div><strong>Название: </strong>' + data_name + '</div>');
      address = $('<div><strong>Адрес: </strong>' + data_address + '</div>');
      site = $('<div><strong>Сайт: </strong>' + data_site + '</div>');
      social = $('<div><strong>Социальные сети: </strong>' + data_social + '</div>');
      
      center = [region, year, name, address, site, social ];
      
      holder = $('<article class="listed__centres"></article>');
      holder.append(center);
      
      article.push(holder);
      
      
      pagination = $("<section>", { // Создатём елемент
         id: 'block_' + fd_name
      });
      
      pagination = $('<section></section>');
      pagination.prop( 'id', 'block_' + fd_name);
      // console.log(pagination);
      
      pagination.append(article);
      
      $('#listed__holder').append(pagination);
  }*/
  function list( data_region, data_year, data_name, data_address, data_site, data_social, fd) {
      ...
      holder = $("<article>", { // Создатём елемент
         class: 'listed__centres '+ fd,
      });
      holder.append(center);
      
      $('#listed__holder').append(holder);
   ...
   $districts.select2({ // Преобразуем селект #districts в select2 и заполняем данными из json
      data: districts,
      "language": {
         "noResults": function () {
   ...
   $regions.select2({ // Преобразуем селект #regions в select2 и заполняем данными из json со всеми регионами
      data: regions,
      "language": {
         "noResults": function () {
            return "Результаты не найдены";
   ...
   // ОТРИСОКВА ВСЕХ ЦЕНТРОВ IT-КУБ
   
   function totalNumber(total_amount) { // Надпись сверху с общим числом
      $h2.text(total_amount);
      $word.text(getNoun(total_amount, 'центр', 'центра', 'центров'));
   }
   
   // ОСНОВНАЯ ФУНКЦИЯ ПО ОТРИСОВКЕ СПИСКА ЦЕНТРОВ
   function showCenters( regions_array, line ) {
      clearListed();
      total_amount = 0;
      total = [];
      let data_centres = [],
      ...
      $h1.text(line);
       // Перебираем массив регионов
      $.each(regions_array, function (index, data) {
            fd = data.federalDistrict;
         $(data.Centres).each( function(index, data) {
            
            list( data.region, data.year, data.name, data.address, data.site, data.social, fd);
            
             total.push(1);
         });
         
      });
      total_amount = total.reduce(function (a, b) { // Считаем сумму всех объектов инфраструктуры
         return a + b;
      });
   ...
   function showEmptyDistrict( line, place ) {
      clearListed();
      $h1.text(line);
      totalNumber('0');
      
      $('#listed__holder').append('<article class="empty_district"><h1>В данном ' + place + ' нет центров &laquo;IT-куб&raquo;</h1></article>');
   }
   
   // ВЫБИРАЕМ ЧЕРЕЗ SELECT ОДИН ФО
   
   $districts.on('select2:select', function (event) { // Выбор одного ФО
             clearListed(); // должна быть очистка всего и вся

      $regions.prop("disabled", true);
      $select_clear.removeClass('disable'); // Разблокируем кнопку очистки выбора
      $back__holder.css( 'display', 'flex'); // Появление кнопки "все регионы"
      
      let federalDistrict = event.params.data.federalDistrict;
      let title = event.params.data.text;
      showChosenFORegions ( federalDistrict, title ); // Затемняем все регионы в выбранном ФО
      
   });

   // Затемняем все регионы в выбранном ФО
   function showChosenFORegions ( federalDistrict, title ) {
      let chosen_id,
          regions_array = [];
          total = 0;
      $('.chosen_region').removeClass('chosen_region');
      $.each(regions, function (index, data) { // Перебираем json всех регионов, выбираем нужные
         let fd = data.federalDistrict;
         regions_array = [];
         
         if ( fd == federalDistrict ) { // Выбираем регионы в выбранном ФО
            chosen_id = data.id;
            $('#' + chosen_id).addClass('chosen_region'); // Затемняем регионы в выбранном ФО
            
            if ( $('#' + chosen_id).data( 'total' ) > 0 ) { // Отправляем в отрисовку только регионы в выбранном ФО
               regions_array.push(data);
               
               showCenters( regions_array, title );
               
               total += 1;
            }
            if (total == '0') { // Если нет регионов, с центрами в данном ФО
               showEmptyDistrict( title, 'федеральном округе' );
            }
         }
      });
   }
  
   // ВЫБИРАЕМ ЧЕРЕЗ SELECT ОДИН РЕГИОН
   
   $regions.on('select2:select', function (event) { // Выбор одного региона
      clearListed();
   
      $('#' + event.params.data.id).addClass('chosen_region'); // Выделяем выбранный регион

      $districts.prop("disabled", true);
      $select_clear.removeClass('disable');
      $back__holder.css( 'display', 'flex');
   
      let regions_array = event.params.data.Centres,
         line = event.params.data.text,
         total = event.params.data.total;
      
      if (regions_array) {
         showListInRegion( regions_array, line, total);
      } else {
         showEmptyDistrict( line, 'регионе' );
      }

   });
   
    function showListInRegion(regions_array, line, total) {
       clearListed();
       $h1.text(line);
        
      $.each(regions_array, function (index, data) {
            
         list( data.region, data.year, data.name, data.address, data.site, data.social);
