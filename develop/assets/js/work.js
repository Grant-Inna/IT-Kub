$(document).ready(function () {
   ...
   function goTo(event) {
      ...
      $('body, html').animate({ scrollTop: $(id).offset().top - 50 }, speed); // чуть выше самого блока
   }
   ...
   /*  карта и манипуляции с ней  */
   
  
   const regions = [
      {
         text: 'Республика Адыгея',
         federalDistrict: 'district_2',
         id: 87,
      ...
      },
      {
         ...
         federalDistrict: 'district_3',
         id: 52,
         path: 'M305.6,394.7l-2.1,2.1l-3.5,1l-3,1.6l-1.4-2l-2.7,1.3h-2.2 l0.8-3.4l3-1.4v-1.8H292l-2.3-1.4l-1.4,1.4l-2.5-0.8l0.8-3l-1.8-1.8l-2.5,1.5l-2.8-0.8l-0.7-2.2l1.6-1.6v-2.4l0.6-1.8l-1-3.2 l-1.9-0.9v-1.5l0.7-1.5l1.3-1.3v-1.7l1-0.2l1.3-1.3l0.5-1.3v-2.2l3.2-3.2l2.4-0.4l0.9-0.9V358l-1.9-1l-0.6-1.5l-1.2-1.2l-1.7-0.9 V352l2-1l-0.9-2.6l0.2-2l1.8-2l0.9-1.5l1.9-1.3l2,2l-1.2,1.2v1.5l1.7,0.6l2.2,0.7l1.4,0.7l1.6,1h2.6l0.9-1.2l2.2,1.1l1.9,0.6h3 l2.5-1.2l2-2l1.1-1.6l0.4,1.2l-0.7,1.6l-0.8,1.7v2.6l1.5,1.5l-0.4,3.2l1.4,2.9l0.8,2.2v3.2v2.9l-2,2l0.4,1.4v1.9l-1,1l-1.8-0.7 l-1.6,0.7l-2.4,2.8l-1.4,3.5l0.6,4.5v2.6l1.3,1.3v1.5l-1.4,1.4L305.6,394.7',
         total: 0,
         Centres: null
      },
      {
      ...
      },
      {
         ...
         federalDistrict: 'district_1',
         id: 75,
         path: 'M105.1,208.8c0,0.1,0,0.1-0.1,0.1c-0.1,0.1,0,0.1,0,0.2\
      ...
      },
      {
         ...
         federalDistrict: 'district_2',
         id: 82,
         path: 'M-2.6,338.6H3l1.2,3.8c0,0.1,1,3.1-3.1,3.1c-0,3.1z',
         total: 0,
         Centres: null
      }
   ];
   ...
   var federalDistrict_1 = document.createElementNS("http://www.w3.org/2000/svg","g");
   var federalDistrict_2 = document.createElementNS("http://www.w3.org/2000/svg","g");
   var federalDistrict_3 = document.createElementNS("http://www.w3.org/2000/svg","g");
   var federalDistrict_0 = document.createElementNS("http://www.w3.org/2000/svg","g");
   ...
   // Перебираем массив регионов
      $.each(regions, function (index, data) {
         ...
         if ( data.federalDistrict === 'district_1' ) {
            newpath.setAttribute("fill", "#FA90AF");
            federalDistrict_1.setAttribute("id", data.federalDistrict);
            federalDistrict_1.appendChild(newpath);
            
            
         } else if ( data.federalDistrict === 'district_2' ) {
            newpath.setAttribute("fill", "#F0CFA7");
            federalDistrict_2.setAttribute("id", data.federalDistrict);
            federalDistrict_2.appendChild(newpath);
            
         } else if ( data.federalDistrict === 'district_3' ) {
            newpath.setAttribute("fill", "#83a3e3");
            federalDistrict_3.setAttribute("id", data.federalDistrict);
            federalDistrict_3.appendChild(newpath);
            
         } else if ( data.federalDistrict === 'district_0' ) {
            newpath.setAttribute("fill", "#DDE5EF");
            federalDistrict_0.setAttribute("id", data.federalDistrict);
            federalDistrict_0.appendChild(newpath);
         }
        
         document.getElementById("map_index__svg").appendChild(federalDistrict_1);
         document.getElementById("map_index__svg").appendChild(federalDistrict_2);
         document.getElementById("map_index__svg").appendChild(federalDistrict_3);
         document.getElementById("map_index__svg").appendChild(federalDistrict_0);
      });
   }
   
   if ($('#map_index__svg').length > 0) {
      initMap(regions);
      
      $('.map_index__information').hide();
      
      $('#map_index__svg').find('g').on( 'mouseenter', (event) => { // Показываем нужную карточку
         
         let id = $(event.target).closest('g').prop('id').split('_')[1];
         $('.map_index__information').not(('.map_index__information#card_' + id)).hide();
         $('.map_index__information#card_' + id).show();
      });
      // console.log($('#map_index__svg').offset()); // вывод результата в консоль
   
      $('#map_index__svg').on( 'mousemove', (event) => { // Прилепляем её к курсору
         var pos = $(event.target).closest('.prelative').offset(),
             width = Number($(event.target).closest('.prelative').outerWidth());
         ...
         var Yinner = Number(event.pageY - elem_top + 10); // - положение элемента
         
         if ((width - 250) <  event.pageX) {
            $('.map_index__information').css({'top': Yinner, 'left': Xinner - 200});
            if ($(document).width() < 580) {
               $('.map_index__information').css({'top': Yinner, 'left': Xinner - 150})
            }
         } else {
            $('.map_index__information').css({'top': Yinner, 'left': Xinner})
         }
      ...
      $('#map_index__svg').on( 'mouseleave', (event) => {
         $('.map_index__information').hide();
      });
   }
